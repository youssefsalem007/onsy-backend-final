...
export const triggerRealtimeAnalysis = (userId, eegSessionId = null) => {
  return Promise.all([
    ...
    eegSessionId
      ...
      : EEGReading.findOne({ userId })
          .sort({ timestamp: -1 })
          .then((latest) =>
            latest
              ? EEGReading.find({ sessionId: latest.sessionId, userId })
              : [],
          ),
    ...
    .then(([moodLogs, eegReadings, recentSession]) => {
      ...
      let sessionId =
        eegSessionId ||
        (eegReadings.length > 0 ? eegReadings[0].sessionId : null);
      ...
      if (eegReadings && eegReadings.length > 0) {
        ...
        for (const reading of eegReadings) {
          if (
            reading.metrics?.excitement !== null &&
            reading.metrics?.excitement !== undefined
          ) {
            sums.excitement += reading.metrics.excitement;
            counts.excitement++;
          }
          if (
            reading.metrics?.stress !== null &&
            reading.metrics?.stress !== undefined
          ) {
            sums.stress += reading.metrics.stress;
            counts.stress++;
          }
          if (
            reading.metrics?.focus !== null &&
            reading.metrics?.focus !== undefined
          ) {
            sums.focus += reading.metrics.focus;
            counts.focus++;
          }
          if (
            reading.metrics?.relaxation !== null &&
            reading.metrics?.relaxation !== undefined
          ) {
            sums.relaxation += reading.metrics.relaxation;
            counts.relaxation++;
          }
          if (
            reading.metrics?.engagement !== null &&
            reading.metrics?.engagement !== undefined
          ) {
            sums.engagement += reading.metrics.engagement;
            counts.engagement++;
          }
          if (
            reading.metrics?.interest !== null &&
            reading.metrics?.interest !== undefined
          ) {
            sums.interest += reading.metrics.interest;
            counts.interest++;
          }
          if (
            reading.metrics?.longTermExcitement !== null &&
            reading.metrics?.longTermExcitement !== undefined
          ) {
            sums.longTermExcitement += reading.metrics.longTermExcitement;
            counts.longTermExcitement++;
          }
        }
      ...
      return axios
        .post(
          `${process.env.AI_ENGINE_URL || "http://localhost:8000"}/analyze/realtime`,
          {
            userId,
        ...
        .catch((err) => {
          console.warn(
            "Realtime AI engine unavailable, using fallback. Error:",
            err.message,
          );
          return generateFallbackRealtimeAnalysis(
            eegMetrics,
            mappedMoodLogs,
            recentMessages,
          );
        ...
        .then((savedAnalysis) => {
          try {
            getIO()
              .to(userId.toString())
              .emit("analysis:update", savedAnalysis);
          } catch (ioErr) {
...
function generateFallbackRealtimeAnalysis(eeg, logs, messages) {
  ...
  let recs = [
    ...
    "Consider taking a short 5-minute walk.",
  ];
  ...
  if (logs && logs.length > 0) {
    ...
    if (recent.mood <= 4) {
      ...
      dom = "sadness";
      recs.push(
        "We noticed you logged a negative mood. Consider reaching out to a friend.",
      );
      ...
      recs = [
        "We noticed you logged a positive mood. Keep it up!",
        "Consider sharing your positive experience with a friend.",
      ];
    }
  }
  ...
  if (messages && messages.length > 0) {
    ...
    const isSad = [
      ...
      "empty",
      "lonely",
    ].some((w) => new RegExp(`\\b${w}\\b`, 'i').test(recentText));
    const isAnx = [
      "anxious",
      "worried",
      "panic",
    ...
    const isStress = ["stressed", "overwhelmed", "tired", "exhausted"].some(
      (w) => new RegExp(`\\b${w}\\b`, 'i').test(recentText)
    );
    const isHappy = ["happy", "good", "great", "excited", "joy", "glad"].some(
      (w) => new RegExp(`\\b${w}\\b`, 'i').test(recentText)
    );
    const isRisk = [
      "suicide",
  ...
  let happiness =
    level === 1 && dom === "happiness" ? 0.7 : level === 1 ? 0.4 : 0.05;
